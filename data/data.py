...
class FLOOD(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    # Decode mask as grayscale [1, H, W]
    masks = decode_image(self.mask_dir + f'/{idx}.png', mode='GRAY')
    # Apply same transforms as image to ensure spatial alignment
    masks = self.transforms(masks)
    # Threshold to binary and convert to uint8 for Mask R-CNN
    masks = (masks > 0.5).to(torch.uint8)
...
def detection_collate(batch):
    ...
    targets = [item[1] for item in batch]   # list of dicts, one per sample
